...
def decompose_rotation_with_yaxis(rotation):
    ...
    angle = np.arccos(min(1.0, np.sum(yaxis * global_y)))
    rot_vec = np.zeros(3)
    if angle > 0:
...
class BVHMotion():
    ...
    def batch_forward_kinematics(self, joint_position=None, joint_rotation=None, frame_id_list=None, root_pos=None, root_quat=None):
        ...
        if root_pos is not None:
            root_pos = root_pos.reshape(1, 3).repeat(
                joint_position.shape[0], axis=0)
            joint_position[:, 0, :] = root_pos
        if root_quat is not None:
            root_quat = root_quat.reshape(1, 4).repeat(
                joint_position.shape[0], axis=0)
    ...
    def forward_kinematics(self, frame_idx, frame_id_list, root_pos, root_quat):
        '''
        利用自身的metadata进行批量前向运动学
        frame_idx: 需要转换的frame_idx
        frame_id_list: 一个list, 表示需要计算的帧的id，如果是None则计算所有帧
        root_pos: (3,)的ndarray, 手动指定root joint的位置
        root_quat: (4,)的ndarray, 手动指定root joint的四元数表示的旋转
        '''
        ...
        frame_idx = frame_id_list.index(frame_idx)
        
        joint_position = joint_position[frame_id_list]
        joint_rotation = joint_rotation[frame_id_list]
        
        num_frames = joint_position.shape[0]
        if root_pos is not None or root_quat is not None:
            old_root_pos = joint_position[frame_idx, 0].copy()
            old_root_quat = joint_rotation[frame_idx, 0].copy()

            delta_pos = np.array([0.0, 0.0, 0.0])
            delta_pos = root_pos - old_root_pos

            delta_rot = R.identity()
            r_old = R.from_quat(old_root_quat)
            r_new = R.from_quat(root_quat)
            delta_rot = r_new * r_old.inv()

            # Apply to all frames
            for i in range(num_frames):
                joint_position[i, 0] += delta_pos
                r_ori = R.from_quat(joint_rotation[i, 0])
                joint_rotation[i, 0] = (delta_rot * r_ori).as_quat()
    ...
    def translation_and_rotation(self, frame_num, target_translation_xz, target_facing_direction_xz):
        ...
        offset = target_translation_xz - \
            res.joint_position[frame_num, 0, [0, 2]]
        res.joint_position[:, 0, [0, 2]] += offset

        Ry, Rxz = decompose_rotation_with_yaxis(
            res.joint_rotation[frame_num, 0, :])
        axis = np.array([0, 1, 0]).reshape(3,)
        angle = np.arctan2(
            target_facing_direction_xz[0], target_facing_direction_xz[1])
        ...
        pos_offset = res.joint_position[:, 0, :] - \
            res.joint_position[frame_num, 0, :]
        rotated_pos_offset = rot_diff.apply(pos_offset)
        res.joint_position[:, 0, :] = rotated_pos_offset + \
            res.joint_position[frame_num, 0]
        return res
