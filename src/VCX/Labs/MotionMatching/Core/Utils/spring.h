...
namespace VCX::Labs::MotionMatching::Core::Utils {
    ...
    inline vec3 damper_exact(vec3 x, vec3 g, float halflife, float dt, float eps = 1e-5f) {
        return lerp(x, g, 1.0f - fast_negexpf((LN2f * dt) / (halflife + eps)));
    }

    inline quat damper_exact(quat x, quat g, float halflife, float dt, float eps = 1e-5f) {
        return quat_slerp_shortest_approx(x, g, 1.0f - fast_negexpf((LN2f * dt) / (halflife + eps)));
    }

    inline float damp_adjustment_exact(float g, float halflife, float dt, float eps = 1e-5f) {
        return g * (1.0f - fast_negexpf((LN2f * dt) / (halflife + eps)));
    }

    inline vec3 damp_adjustment_exact(vec3 g, float halflife, float dt, float eps = 1e-5f) {
        return g * (1.0f - fast_negexpf((LN2f * dt) / (halflife + eps)));
    }

    inline quat damp_adjustment_exact(quat g, float halflife, float dt, float eps = 1e-5f) {
    ...
    inline void inertialize_transition(
        vec3 &     off_x,
        vec3 &     off_v,
        const vec3 src_x,
        const vec3 src_v,
    ...
    inline void inertialize_update(
        vec3 &      out_x,
        vec3 &      out_v,
        vec3 &      off_x,
        vec3 &      off_v,
