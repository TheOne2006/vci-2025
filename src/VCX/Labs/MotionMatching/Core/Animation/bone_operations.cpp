...
namespace VCX::Labs::MotionMatching::Core::Animation {
    ...
    void contact_update(
        ...
        const float dt,
        const float eps) {
        // First compute input contact velocity via finite differences
        vec3 input_contact_velocity =
            (input_contact_position - contact_target) / (dt + eps);
        contact_target = input_contact_position;

        // Update inertializer to progress over time
        inertialize_update(
            contact_position,
            contact_velocity,
            contact_offset_position,
            contact_offset_velocity,
            // If locked we provide contact point and zero velocity,
            // otherwise we provide animated input
            contact_lock ? contact_point : input_contact_position,
            contact_lock ? vec3() : input_contact_velocity,
            halflife,
            dt);

        // If contact point is too far from current input position, need to unlock contact
        bool unlock_contact = contact_lock && (length(contact_point - input_contact_position) > unlock_radius);

        // If contact was previously inactive but is now active, we need to transition to locked contact state
        if (! contact_state && input_contact_state) {
            // Contact point is given by projecting current foot position to ground plus foot height
            contact_lock    = true;
        ...
        // Otherwise, if we need to unlock or were previously in contact but no longer, we transition to using input position directly
