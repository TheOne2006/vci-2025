...
namespace VCX::Labs::MotionMatching::Core::Math {
    ...
    static inline quat quat_exp(vec3 v, float eps = 1e-8f) {
        float halfangle = std::sqrt(v.x * v.x + v.y * v.y + v.z * v.z);

        if (halfangle < eps) {
            return quat_normalize(quat(1.0f, v.x, v.y, v.z));
        } else {
            float c = std::cos(halfangle);
            float s = std::sin(halfangle) / halfangle;
            return quat(c, s * v.x, s * v.y, s * v.z);
        }
    }

    static inline vec3 quat_log(quat q, float eps = 1e-8f) {
        float length = std::sqrt(q.x * q.x + q.y * q.y + q.z * q.z);

        if (length < eps) {
            return vec3(q.x, q.y, q.z);
        } else {
            float halfangle = std::acos(clampf(q.w, -1.0f, 1.0f));
    ...
    static inline void quat_to_angle_axis(quat q, float & angle, vec3 & axis, float eps = 1e-8f) {
        ...
        if (length < eps) {
            ...
            angle = 2.0f * std::acos(clampf(q.w, -1.0f, 1.0f));
    ...
    static inline quat quat_slerp_shortest(quat q, quat p, float alpha, float eps = 1e-5f) {
        ...
        float theta = std::acos(clampf(dot, -1.0f, 1.0f));
        ...
        return q * std::cos(theta * alpha) + r * std::sin(theta * alpha);
    }
    ...
    static inline float quat_angle_between(quat q, quat p) {
        ...
        return 2.0f * std::acos(clampf(diff.w, -1.0f, 1.0f));
    }
    ...
    static inline quat quat_between(vec3 p, vec3 q) {
        ...
        return quat_normalize(quat(
            std::sqrt(dot(p, p) * dot(q, q)) + dot(p, q),
            c.x,
            c.y,
            c.z));
    }
